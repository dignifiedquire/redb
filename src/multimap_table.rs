...
impl<'a, V: RedbKey + 'static, F: Fs> MultimapValue<'a, V, F> {
    ...
    fn new_inline(inner: LeafKeyIter<'a, F>) -> Self {
        Self {
...
pub struct MultimapTable<'db, 'txn, K: RedbKey + 'static, V: RedbKey + 'static, F: Fs> {
    ...
    freed_pages: Arc<Mutex<Vec<PageNumber>>>,
    tree: BtreeMut<'txn, K, &'static DynamicCollection, F>,
        ...
        V: 'a,
    {
        ...
        let existed = if get_result.is_some() {
            ...
            match collection_type {
                Inline => {
                    ...
                    if required_inline_bytes < self.mem.get_page_size() / 2 {
                        ...
                        let mut subtree: BtreeMut<'_, V, (), F> = BtreeMut::new(
                            Some((page_number, 0)),
                            self.mem,
            ...
            if required_inline_bytes < self.mem.get_page_size() / 2 {
                ...
                let mut subtree: BtreeMut<'_, V, (), F> =
    ...
    for MultimapTable<'db, 'txn, K, V, F>
{
    ...
    fn range<'a, KR>(&self, range: impl RangeBounds<KR> + 'a) -> Result<MultimapRange<K, V, F>>
    where
        K: 'a,
...
pub trait ReadableMultimapTable<K: RedbKey + 'static, V: RedbKey + 'static, F: Fs>: Sealed {
    ...
    /// order.
    fn iter(&self) -> Result<MultimapRange<K, V, F>> {
...
impl<'txn, K: RedbKey + 'static, V: RedbKey + 'static, F: Fs> ReadOnlyMultimapTable<'txn, K, V, F> {
    pub(crate) fn new(
        ...
        hint: PageHint,
        mem: &'txn TransactionalMemory<F>,
    ) -> Result<ReadOnlyMultimapTable<'txn, K, V, F>> {
