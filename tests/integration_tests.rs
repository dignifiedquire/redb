...
fn multi_page_kv() {
    ...
    let db = Builder::<StdFs>::new().create(tmpfile.path()).unwrap();
    let txn = db.begin_write().unwrap();
...
fn does_not_exist() {
    ...
    let result = Database::<StdFs>::open(tmpfile.path());
...
fn database_lock() {
    let tmpfile: NamedTempFile = NamedTempFile::new().unwrap();
    let result = Database::<StdFs>::create(tmpfile.path());
    assert!(result.is_ok());
    let result2 = Database::<StdFs>::open(tmpfile.path());
    assert!(
...
fn compaction() {
    ...
    drop(db);
    let file_size = tmpfile.as_file().metadata().unwrap().len();
    let mut db = Database::<StdFs>::open(tmpfile.path()).unwrap();
